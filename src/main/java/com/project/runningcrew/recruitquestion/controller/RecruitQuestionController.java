...
import com.project.runningcrew.recruitquestion.dto.request.RecruitQuestionsDto;
...
//    }
    ...
    @Operation(summary = "가입 질문들 설정하기", description = "기존 가입 질문들은 삭제하고, 입력받은 가입 질문들로 새로 설정한다.", security = {@SecurityRequirement(name = "Bearer-Key")})
    @ApiResponses({
            @ApiResponse(responseCode = "204", description = "NO CONTENT", content = @Content()),
            @ApiResponse(responseCode = "401", description = "UNAUTHORIZED",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponse.class))),
    ...
    @PutMapping("/api/crew/{crewId}/recruit-questions")
    public ResponseEntity<Void> setRecruitQuestions(@PathVariable("crewId") Long crewId,
                                                    @Parameter(hidden = true) @CurrentUser User user,
                                                    @RequestBody @Valid RecruitQuestionsDto recruitQuestionsDto
    ) {
        Crew crew = crewService.findById(crewId);
        CrewCondition crewCondition = crewConditionService.findByCrew(crew);
        if (!crewCondition.isJoinQuestion()) {
        ...
        List<RecruitQuestion> recruitQuestions = recruitQuestionsDto.getQuestions().stream()
                .map(r -> new RecruitQuestion(crew, r.getQuestion(), r.getQuestionOffset()))
