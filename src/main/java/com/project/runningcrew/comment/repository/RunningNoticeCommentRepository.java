...
public interface RunningNoticeCommentRepository extends JpaRepository<RunningNoticeComment, Long> {
    ...
    @Query("select new com.project.runningcrew.common.dto.SimpleCommentDto(rc.id, m.id, rc.createdDate, rc.detail, u.nickname, u.imgUrl) " +
            ...
            "inner join RunningNotice r on r = rc.runningNotice and r.deleted = false " +
            "inner join Member m on m = rc.member and m.deleted = false " +
     ...
     */
    @Modifying
    @Query("update RunningNoticeComment rc " +
            "set rc.deleted = true " +
            "where rc.runningNotice = :runningNotice")
     ...
     */
    @Query("select rc.runningNotice.id, count(rc) " +
            "from RunningNoticeComment rc " +
            "where rc.runningNotice.id in (:runningNoticeIds) " +
            "group by rc.runningNotice.id")
